...
function ConvertTo-RegistryProviderPath {
  ...
  # Collapse duplicate separators, trim trailing backslash, and remove
  # trailing colon only when it follows a path separator (preserving bare
  # PS drive names such as 'HKLM:').
  $_normalized = $_normalized -replace '\\+', '\'
  if ($_normalized -match '\\') {
    $_normalized = $_normalized -replace ':+$', ''
  }
  $_normalized = $_normalized -replace '\\+$', ''
